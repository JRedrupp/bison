...
def test_eval_int_gt() raises:
    ...
    var _dp = mask.to_pandas()
    assert_true(not Bool(py=_dp.iloc[0]))
    assert_true(not Bool(py=_dp.iloc[1]))
    assert_true(Bool(py=_dp.iloc[2]))
    assert_true(Bool(py=_dp.iloc[3]))
...
def test_eval_string_eq() raises:
    ...
    var _dp = mask.to_pandas()
    assert_true(Bool(py=_dp.iloc[0]))
    assert_true(not Bool(py=_dp.iloc[1]))
    assert_true(Bool(py=_dp.iloc[2]))
    assert_true(not Bool(py=_dp.iloc[3]))
...
def test_eval_literal_left() raises:
    ...
    var _dn_pd = mask_normal.to_pandas()
    var _df2_pd = mask_flipped.to_pandas()
    for i in range(3):
        assert_equal(Bool(py=_dn_pd.iloc[i]), Bool(py=_df2_pd.iloc[i]))
...
def test_eval_all_numeric_ops() raises:
    ...
    assert_true(Bool(py=lt.to_pandas().iloc[0]))
    assert_true(not Bool(py=lt.to_pandas().iloc[1]))
    assert_true(not Bool(py=lt.to_pandas().iloc[2]))
    # x <= 2 → [T, T, F]
    var le = eval_expr(parse("x <= 2"), df)
    assert_true(Bool(py=le.to_pandas().iloc[0]))
    assert_true(Bool(py=le.to_pandas().iloc[1]))
    assert_true(not Bool(py=le.to_pandas().iloc[2]))
    # x > 2  → [F, F, T]
    var gt = eval_expr(parse("x > 2"), df)
    assert_true(not Bool(py=gt.to_pandas().iloc[0]))
    assert_true(not Bool(py=gt.to_pandas().iloc[1]))
    assert_true(Bool(py=gt.to_pandas().iloc[2]))
    # x >= 2 → [F, T, T]
    var ge = eval_expr(parse("x >= 2"), df)
    assert_true(not Bool(py=ge.to_pandas().iloc[0]))
    assert_true(Bool(py=ge.to_pandas().iloc[1]))
    assert_true(Bool(py=ge.to_pandas().iloc[2]))
    # x == 2 → [F, T, F]
    var eq = eval_expr(parse("x == 2"), df)
    assert_true(not Bool(py=eq.to_pandas().iloc[0]))
    assert_true(Bool(py=eq.to_pandas().iloc[1]))
    assert_true(not Bool(py=eq.to_pandas().iloc[2]))
    # x != 2 → [T, F, T]
    var ne = eval_expr(parse("x != 2"), df)
    assert_true(Bool(py=ne.to_pandas().iloc[0]))
    assert_true(not Bool(py=ne.to_pandas().iloc[1]))
    assert_true(Bool(py=ne.to_pandas().iloc[2]))
...
def test_eval_column_vs_column() raises:
    ...
    assert_true(not Bool(py=gt.to_pandas().iloc[0]))
    assert_true(Bool(py=gt.to_pandas().iloc[1]))
    assert_true(not Bool(py=gt.to_pandas().iloc[2]))
    # a == b → [F, F, T]
    var eq = eval_expr(parse("a == b"), df)
    assert_true(not Bool(py=eq.to_pandas().iloc[0]))
    assert_true(not Bool(py=eq.to_pandas().iloc[1]))
    assert_true(Bool(py=eq.to_pandas().iloc[2]))
    # a != b → [T, T, F]
    var ne = eval_expr(parse("a != b"), df)
    assert_true(Bool(py=ne.to_pandas().iloc[0]))
    assert_true(Bool(py=ne.to_pandas().iloc[1]))
    assert_true(not Bool(py=ne.to_pandas().iloc[2]))
    # a <= b → [T, F, T]
    var le = eval_expr(parse("a <= b"), df)
    assert_true(Bool(py=le.to_pandas().iloc[0]))
    assert_true(not Bool(py=le.to_pandas().iloc[1]))
    assert_true(Bool(py=le.to_pandas().iloc[2]))
    # a >= b → [F, T, T]
...
def test_eval_and() raises:
    ...
    var _dp = mask.to_pandas()
    assert_true(not Bool(py=_dp.iloc[0]))
    assert_true(Bool(py=_dp.iloc[1]))
    assert_true(Bool(py=_dp.iloc[2]))
    assert_true(not Bool(py=_dp.iloc[3]))
...
def test_eval_or() raises:
    ...
    assert_true(Bool(py=_dp.iloc[0]))
    assert_true(not Bool(py=_dp.iloc[1]))
    assert_true(not Bool(py=_dp.iloc[2]))
    assert_true(Bool(py=_dp.iloc[3]))
...
def test_eval_null_and() raises:
    ...
    assert_true(not Bool(py=mask.to_pandas().iloc[0]))
    assert_true(mask._col.is_null(1))
    assert_false(mask._col.is_null(2))
    assert_true(Bool(py=mask.to_pandas().iloc[2]))
...
def test_eval_null_or() raises:
    ...
    assert_true(mask._col.has_nulls())
    assert_false(mask._col.is_null(0))
    assert_true(Bool(py=mask.to_pandas().iloc[0]))
    assert_true(mask._col.is_null(1))
    assert_false(mask._col.is_null(2))
    assert_true(Bool(py=mask.to_pandas().iloc[2]))
...
def test_query_simple_numeric() raises:
    ...
    var col_a = result["a"]
    var _col_a_pd = col_a.to_pandas()
    assert_true(atof(String(_col_a_pd.iloc[0])) > 0.5)
    assert_true(atof(String(_col_a_pd.iloc[1])) > 0.5)
...
def test_conformance_query_scalar_lt() raises:
    ...
    var _bs_a_pd = bs_a_series.to_pandas()
    for i in range(expected_n):
        assert_equal(Int(py=_bs_a_pd.iloc[i]), Int(py=pd_vals[i]))
...
def test_conformance_query_float_scalar() raises:
    ...
    var _bs_y_pd = bs_y_series.to_pandas()
    for i in range(expected_n):
        assert_true(abs(atof(String(_bs_y_pd.iloc[i])) - atof(String(pd_vals[i]))) < 1e-9)
...
def test_conformance_query_string_eq() raises:
    ...
    var _bs_vals_pd = bs_val_series.to_pandas()
    for i in range(expected_n):
        assert_equal(Int(py=_bs_vals_pd.iloc[i]), Int(py=pd_vals[i]))
...
def test_conformance_query_nulls_excluded() raises:
    ...
    var _bs_a_pd = bs_a_series.to_pandas()
    for i in range(expected_n):
        assert_true(abs(atof(String(_bs_a_pd.iloc[i])) - atof(String(pd_vals[i]))) < 1e-9)
...
def test_conformance_eval_scalar_lt() raises:
    ...
    var _bs_d_pd = bs_mask.to_pandas()
    assert_equal(Int(py=_bs_d_pd.__len__()), 5)
    for i in range(5):
        assert_equal(Bool(py=_bs_d_pd.iloc[i]), Bool(py=pd_bools[i]))
...
def test_conformance_eval_and() raises:
    ...
    var _bs_d_pd = bs_mask.to_pandas()
    assert_equal(Int(py=_bs_d_pd.__len__()), 4)
    for i in range(4):
        assert_equal(Bool(py=_bs_d_pd.iloc[i]), Bool(py=pd_bools[i]))
...
def test_conformance_eval_chained_and() raises:
    ...
    var _bs_d_pd = bs_mask.to_pandas()
    assert_equal(Int(py=_bs_d_pd.__len__()), 3)
    for i in range(3):
        assert_equal(Bool(py=_bs_d_pd.iloc[i]), Bool(py=pd_bools[i]))
...
def test_conformance_eval_nulls_simple() raises:
    ...
    assert_true(not Bool(py=bs_mask.to_pandas().iloc[0]))
    assert_false(bs_mask._col.is_null(2))
    assert_true(Bool(py=bs_mask.to_pandas().iloc[2]))
    assert_false(bs_mask._col.is_null(4))
    assert_true(Bool(py=bs_mask.to_pandas().iloc[4]))
